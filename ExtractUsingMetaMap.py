...
if __name__=="__main__":
    ...
    for res in results:
        ...
        for res in rowRes:
            ...
            if("mean" in m2.keys()):
                queryclass.SaveExtracted(id,TableID,TableOrder,PMC,AnnotationContent,"mean",float(m2["mean"]),unit,res[10],"MetaMap","MetaMap") 
            if("sd" in m2.keys()):
                queryclass.SaveExtracted(id,TableID,TableOrder,PMC,AnnotationContent,"sd",float(m2["sd"]),unit,res[10],"MetaMap","MetaMap") 
            if("min" in m2.keys()):
                queryclass.SaveExtracted(id,TableID,TableOrder,PMC,AnnotationContent,"Range:minimum",float(m2["min"]),unit,res[10],"MetaMap","MetaMap") 
            if("max" in m2.keys()):
                queryclass.SaveExtracted(id,TableID,TableOrder,PMC,AnnotationContent,"Range:maximum",float(m2["max"]),unit,res[10],"MetaMap","MetaMap")  
