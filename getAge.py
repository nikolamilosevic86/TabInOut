    ...
    for id in articleIds:
        ...
        for t in results:
            ...
            for res in resulta:
                ...
                if m3==None:
                    continue
                if(m2==None or m2.group(0)=='.'or m2.group(0)=='..'):
                    continue
                content = re.sub(r'[^\x00-\x7F]','[spec]',res[9])
                content = content.replace('?','[spec]')
                if "range" in res[10].lower() or ("sd" in res[10].lower() and 'mean' not in res[10].lower()) or "p-value" in res[10].lower() or "p" ==res[10].lower():
                    continue
            ...
            for res in resulta:
                ...
                if m3==None:
                    continue
                if (("median" not in res[11].lower() or "mean" not in res[11].lower()) and "range" in res[11].lower()) or ("sd" in res[11].lower() and "mean" not in  res[11].lower() ) or "%" in res[11].lower():
                    continue
                content = re.sub(r'[^\x00-\x7F]','[spec]',res[9])
                content = content.replace('?','[spec]')
                if('<' in content or '>' in content or '=' in content or '?' in content or '<' in res[11] or '>' in res[11] or '=' in res[11]or '?' in res[11]or 'min' in res[11].lower() or 'max' in res[11].lower()):
                    continue
                totalNum = float(m2.group(0))
                unit = 'years'
                if("months" in res[11].lower() or "months" in res[9].lower()):
                ...
                if("weeks" in res[11].lower() or "weeks" in res[9].lower()):
                    #totalNum = totalNum/52
                    unit = 'weeks'
                if "range" in res[10].lower() or "sd" in res[10].lower() or "p-value" in res[10].lower() or "p" ==res[10].lower():
                    continue
                print 'Age:'+str(totalNum) + " "+unit
                queryclass.SaveAttribute(id,"Mean",table.tableId,table.tableOrder,PMCs[l],"Age","",totalNum,unit,res[10]) 
        l = l+1        
    print "Done"